...
A `NetClass` can claim nets explicitly via its `nets` field, listing net names or KiCad wildcard patterns. Explicit assignments take precedence over impedance auto-matching.

```python
NetClass(name="Power", track_width=0.5, clearance=0.2, nets=["VCC_5V", "GND_PWR", "V48*"])
```
