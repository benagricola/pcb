...
/// PCB stackup layer - either copper or dielectric.
///
/// Custom deserialize impl required because `#[serde(untagged)]` doesn't work
/// reliably with struct variants. We distinguish by presence of "role" (Copper)
/// vs "material"/"form" (Dielectric).
...
#[derive(Deserialize)]
struct CopperLayerData {
    thickness: f64,
    role: CopperRole,
}

#[derive(Deserialize)]
struct DielectricLayerData {
    thickness: f64,
    material: String,
    form: DielectricForm,
}
...
impl<'de> Deserialize<'de> for Layer {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        if obj.contains_key("role") {
            serde_json::from_value::<CopperLayerData>(value)
                .map(|d| Layer::Copper {
            ...
            serde_json::from_value::<DielectricLayerData>(value)
                .map(|d| Layer::Dielectric {
                    thickness: d.thickness,
                    material: d.material,
                    form: d.form,
                })
...
"#;
    ...
    #[test]
    fn test_layer_json_round_trip() {
        // Test that Layer serializes/deserializes correctly (regression test for untagged enum issue)
        let copper_layer = Layer::Copper {
            thickness: 0.035,
            role: CopperRole::Mixed,
        };
        let copper_json = serde_json::to_string(&copper_layer).unwrap();
        let copper_parsed: Layer = serde_json::from_str(&copper_json).unwrap();
        assert_eq!(copper_layer, copper_parsed);

        let dielectric_layer = Layer::Dielectric {
            thickness: 0.2104,
            material: "Prepreg".to_string(),
            form: DielectricForm::Prepreg,
        };
        let dielectric_json = serde_json::to_string(&dielectric_layer).unwrap();
        let dielectric_parsed: Layer = serde_json::from_str(&dielectric_json).unwrap();
        assert_eq!(dielectric_layer, dielectric_parsed);

        // Test parsing layers array (as generated by Zen stdlib)
        let layers_json = r#"[
        ...
        let layers: Vec<Layer> = serde_json::from_str(layers_json).unwrap();
